...
namespace orc {
  ...
  int32_t TypeImpl::assignIds(int32_t root) {
    columnId = root;
    int32_t current = root + 1;
  ...
  int32_t TypeImpl::getColumnId() const {
    return columnId;
  }
  ...
  std::unique_ptr<Type> convertType(const proto::Type& type,
    ...
    switch (static_cast<int32_t>(type.kind())) {
    ...
    case proto::Type_Kind_UNION: {
      ...
      for(int32_t i=0; i < type.subtypes_size(); ++i) {
        typeList[static_cast<uint64_t>(i)] =
          convertType(footer.types(static_cast<int32_t>(type.subtypes(i))),
                      footer).release();
      }
      return std::unique_ptr<Type>
