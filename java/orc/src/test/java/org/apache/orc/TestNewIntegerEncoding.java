...
public class TestNewIntegerEncoding {
	...
	public void testBasicRow() throws Exception {
		...
		schema.addField("f1", TypeDescription.createInt());
		schema.addField("f2", TypeDescription.createLong());
		...
		OrcStruct struct = new OrcStruct(2);
		struct.setFieldValue(0, new IntWritable(111));
		struct.setFieldValue(1, new LongWritable(1111L));
		writer.addRow(struct);

		struct = new OrcStruct(2);
		struct.setFieldValue(0, new IntWritable(111));
		struct.setFieldValue(1, new LongWritable(1111L));
		writer.addRow(struct);
	...
	public void testBasicOld() throws Exception {
		...
		Writer writer = OrcFile.createWriter(testFilePath,
				...
				.encodingStrategy(encodingStrategy));
		for(Long l : input) {
			writer.addRow(new LongWritable(l));
		}
		writer.close();
	...
	public void testPatchedBaseTimestamp() throws Exception {
		TypeDescription schema =  TypeDescription.createStruct();
		schema.addField("ts", TypeDescription.createTimestamp());
		...
		for (Timestamp ts : tslist) {
			OrcStruct struct = new OrcStruct(1);
			struct.setFieldValue(0, new TimestampWritable(ts));
			writer.addRow(struct);
		}
	...
	public void testDirectLargeNegatives() throws Exception {
		...
		writer.addRow(new LongWritable(-7486502418706614742L));
		writer.addRow(new LongWritable(0L));
		writer.addRow(new LongWritable(1L));
		writer.addRow(new LongWritable(1L));
		writer.addRow(new LongWritable(-5535739865598783616L));
		writer.close();
