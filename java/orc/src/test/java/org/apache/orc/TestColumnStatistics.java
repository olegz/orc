...
public class TestColumnStatistics {
	...
	public void testHasNull() throws Exception {
		...
		schema.addField("c1", TypeDescription.createBinary());
		schema.addField("c2", TypeDescription.createString());
		...
		for(int i=0; i<1000; i++) {
			...
			struct.setFieldValue(0, bytes(1,2,3));
			struct.setFieldValue(1, new Text("RG1"));
		...
		for(int i=0; i<1000; i++) {
			OrcStruct struct = new OrcStruct(2);
			struct.setFieldValue(0, bytes(1,2,3));
			struct.setFieldValue(1, new Text("RG3"));
			writer.addRow(struct);
		...
		for(int i=0; i<5000; i++) {
			OrcStruct struct = new OrcStruct(2);
			struct.setFieldValue(0, bytes(1,2,3));
			struct.setFieldValue(1, new Text("STRIPE-3"));
			writer.addRow(struct);
