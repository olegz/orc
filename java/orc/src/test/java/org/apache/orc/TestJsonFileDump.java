...
public class TestJsonFileDump {
	...
	public void testJsonDump() throws Exception {
		...
		schema.addField("c1", TypeDescription.createInt());
		schema.addField("c2", TypeDescription.createLong());
		schema.addField("c3", TypeDescription.createString());
		...
		for(int i=0; i < 21000; ++i) {
			OrcStruct struct = new OrcStruct(3);
			if (i % 100 == 0) {
				struct.setFieldValue(0, r1.nextInt());
				struct.setFieldValue(1, r1.nextLong());
				struct.setFieldValue(2, null);
				//				writer.addRow(new MyRecord(r1.nextInt(), r1.nextLong(), null));
			} else {
				struct.setFieldValue(0, r1.nextInt());
				struct.setFieldValue(1, r1.nextLong());
				struct.setFieldValue(2, new Text(words[r1.nextInt(words.length)]));
