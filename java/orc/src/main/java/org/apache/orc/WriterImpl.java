...
import org.apache.hadoop.hive.common.type.HiveDecimal; // storage
...
import org.apache.orc.OrcStruct.Field;
import org.apache.orc.protobuffgen.OrcProto;
import org.apache.orc.protobuffgen.OrcProto.RowIndexEntry;
import org.apache.orc.protobuffgen.OrcProto.StripeStatistics;
import org.apache.orc.protobuffgen.OrcProto.Type;
import org.apache.orc.protobuffgen.OrcProto.UserMetadataItem;
import org.apache.orc.serde.DateWritable;
import org.apache.orc.serde.TimestampWritable;
import org.apache.orc.util.BloomFilterIO;
import org.apache.orc.util.JavaDataModel;
...
public class WriterImpl implements Writer, MemoryManager.Callback {
	...
	public static CompressionCodec createCodec(CompressionKind kind) {
		...
		case LZO:
			try {
				...
				Class.forName("org.apache.hadoop.hive.ql.io.orc.LzoCodec", true, Thread.currentThread().getContextClassLoader());
	...
	private static class BooleanTreeWriter extends TreeWriter {
		...
		void write(Object obj) throws IOException {
			super.write(obj);
			if (obj != null) {
				boolean val = ((BooleanWritable)obj).get();
		...
		void write(Object obj) throws IOException {
			...
			if (obj != null) {
				...
				if (obj instanceof IntWritable){
					val = ((IntWritable)obj).get();
				}
				else if (obj instanceof LongWritable) {
					val = ((LongWritable)obj).get();
				}
				else {
					val = ((ShortWritable)obj).get();
				}
				indexStatistics.updateInteger(val);
				if (createBloomFilter) {
	...
	private static class FloatTreeWriter extends TreeWriter {
		...
		void write(Object obj) throws IOException {
			super.write(obj);
			if (obj != null) {
				float val = ((FloatWritable) obj).get();
	...
	private static class DoubleTreeWriter extends TreeWriter {
		...
		void write(Object obj) throws IOException {
			super.write(obj);
			if (obj != null) {
				double val = ((DoubleWritable) obj).get();
				indexStatistics.updateDouble(val);
				if (createBloomFilter) {
					bloomFilter.addDouble(val);
				}
	...
	private static class BinaryTreeWriter extends TreeWriter {
		...
		void write(Object obj) throws IOException {
			super.write(obj);
			if (obj != null) {
				BytesWritable val = (BytesWritable) obj;
	...
	private static class TimestampTreeWriter extends TreeWriter {
		...
		void write(Object obj) throws IOException {
			super.write(obj);
			if (obj != null) {
				TimestampWritable val = (TimestampWritable) obj;
				Timestamp ts = val.getTimestamp();
				indexStatistics.updateTimestamp(ts);
				seconds.write((ts.getTime() / MILLIS_PER_SECOND) - base_timestamp);
				nanos.write(formatNanos(val.getNanos()));
				if (createBloomFilter) {
					bloomFilter.addLong(ts.getTime());
				}
			}
		}
	...
	private static class DateTreeWriter extends TreeWriter {
		...
		void write(Object obj) throws IOException {
			...
			if (obj != null) {
				...
				DateWritable val = (DateWritable) obj;
	...
	private static class DecimalTreeWriter extends TreeWriter {
		...
		void write(Object obj) throws IOException {
			super.write(obj);
			if (obj != null) {
				HiveDecimal decimal = (HiveDecimal) obj;
	...
	private static class StructTreeWriter extends TreeWriter {
		...
		StructTreeWriter(int columnId,
			...
			//	    	  fields = (List<StructField>) structObjectInspector.getAllStructFieldRefs();
			//	      }
			//	      else {
			List<String> fieldNames = schema.getFieldNames();
			fields = new ArrayList<Field>(fieldNames.size());
		...
		void write(Object obj) throws IOException {
			...
			OrcStruct struct = (OrcStruct) obj;
	...
	private static class ListTreeWriter extends TreeWriter {
		...
		@Override
		void write(Object obj) throws IOException {
			throw new IllegalStateException("Temporary not implemented");
			//      super.write(obj);
			//      if (obj != null) {
