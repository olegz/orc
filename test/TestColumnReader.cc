...
namespace orc {
  ...
  TEST(TestColumnReader, testBooleanWithNulls) {
    ...
    const std::vector<bool> selectedColumns(2,true);
    ...
    {
    unsigned char test[] = {0x3d, 0xf0};
    std::vector<unsigned char> v(test, test+sizeof(test)/sizeof(unsigned char));
    EXPECT_CALL(streams, getStreamProxy(1, proto::Stream_Kind_PRESENT))
      .WillRepeatedly(testing::Return(new SeekableArrayInputStream(v)));
    }

    {
    // [0x0f for x in range(256 / 8)]
    unsigned char test[] = {0x1d, 0x0f};
    std::vector<unsigned char> v(test, test+sizeof(test)/sizeof(unsigned char));
    EXPECT_CALL(streams, getStreamProxy(1, proto::Stream_Kind_DATA))
      .WillRepeatedly(testing::Return(new SeekableArrayInputStream(v)));
    }

    std::vector<Type*> ts(1, createPrimitiveType(BOOLEAN).get());
    std::vector<std::string> vs(1, std::string("col0"));
    // create the row type
    std::auto_ptr<Type> rowType =
      createStructType(ts, vs);
    rowType->assignIds(0);
  ...
  TEST(TestColumnReader, testByteWithNulls) {
    ...
    // create the row type
    std::vector<Type*> ts(1, createPrimitiveType(BYTE).get());
    std::vector<std::string> vs(1, std::string("col0"));
    // create the row type
    std::auto_ptr<Type> rowType =
      createStructType(ts, vs);
...
//    // set getStream
//    EXPECT_CALL(streams, getStreamProxy(0, proto::Stream_Kind_PRESENT))
//      .WillRepeatedly(testing::Return(NULL));
//
//    {
//    unsigned char test[] = {0x19, 0xf0};
//    std::vector<unsigned char> v(test, test+sizeof(test)/sizeof(unsigned char));
//    EXPECT_CALL(streams, getStreamProxy(1, proto::Stream_Kind_PRESENT))
//      .WillRepeatedly(testing::Return(new SeekableArrayInputStream(v)));
//    }
//    {
//    unsigned char test[] = {0x64, 0x01, 0x00};
//    std::vector<unsigned char> v(test, test+sizeof(test)/sizeof(unsigned char));
//    EXPECT_CALL(streams, getStreamProxy(1, proto::Stream_Kind_DATA))
//      .WillRepeatedly(testing::Return(new SeekableArrayInputStream(v)));
//    }
